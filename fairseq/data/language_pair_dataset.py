...
def collate(
    ...
    batch = {
        ...
        "ntokens": ntokens,
        "net_input": {"src_tokens": src_tokens, "src_lengths": src_lengths,"src_distance": src_distance},
        "target": target,
    }
...
class LanguagePairDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        src_distance = self.src_dist.__getitem__(index) if self.src_dist is not None else None
        ...
        """
        s=[]
        for i in example['source']:
            if i!=2:
                s.append(self.src_dict[i])
            print(self.src_dict[i],end=" ")
        print("\n")
        print(example['source_distance'])
        word_list,bpe_info=remove_bpe(s)
        #s = "".join(word_list)
        x=[i+" " for i in word_list]
        output_tree("".join(x))
        """
        if self.align_dataset is not None:
