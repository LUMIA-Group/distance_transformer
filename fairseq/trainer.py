...
class Trainer(object):
    ...
    def _build_optimizer(self):
        distance_params = []
        normal_params = []
        dis_module = ['self_attn.layernorm','self_attn.batchnorm','self_attn.projection1','self_attn.projection2']
        for name,params in self.model.named_parameters():
            flag = False
            for dis in dis_module:
                if dis in name and params.requires_grad:
                    distance_params.append(params)
                    flag = True
                    break
            if not flag:
        ...
        params = list(
            filter(
                ...
                chain(normal_params, self.criterion.parameters()),
            )
        )

        params = [{'params':params} ,{'params':distance_params}]
    ...
    def train_step(self, samples, raise_oom=False, activation_signal=True):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output, cusum = self.task.train_step(
                        sample=sample,
                        model=self.model,
        ...
        if len(cusum_list)>0 and len(cusum_list[0])>0:
            cusum_list = torch.cat(cusum_list,dim=0)
        if is_dummy_batch:
        ...
        return logging_output, cusum_list
