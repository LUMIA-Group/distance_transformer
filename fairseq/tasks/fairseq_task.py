...
class FairseqTask(object):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, activation_signal=True
    ):
        """
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                loss, sample_size, logging_output, cusum = criterion(model, sample, activation_signal=activation_signal)
        if ignore_grad:
            loss *= 0
        with torch.autograd.profiler.record_function("backward"):
            optimizer.backward(loss)
        return loss, sample_size, logging_output, cusum
