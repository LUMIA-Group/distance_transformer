...
def main(cfg: DictConfig) -> None:
    ...
    save_path = os.path.join(root, "distance_prior_new",cfg.task.task_name)
    src_tgt = 'src'
    # ----------------  params domains ----------------#
    ...
    for split in ['train', 'valid', 'test']:
        ...
        # thread_function(2000)
        
        print(split + ' files generated:' + str(len(cnts)) + ', total file:' + str(len(dataset)))
        for i, sent_id in enumerate(tqdm(range(dataset.__len__()))):
            if sent_id == 0:
                print("start from 0")
            if sent_id not in cnts:
                thread_function(sent_id)
                print(str(sent_id) + ' regenerated')
            # if sent_id in lost:
            #     print(str(sent_id) + ' regenerated')
            #     thread_function(sent_id)
